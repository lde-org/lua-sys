...
hook fires (guest interpreter)
  hook_dispatch (C, lua_Hook)
    ...
    build info table on host_L (all debug fields + event + thread + shared mt)
...
All debug fields (`name`, `what`, `source`, `currentline`, ...) are populated eagerly in the info table, along with `event`, `thread`, and a shared metatable (built once at module load, parked in the host registry like the callable metatable) that provides `info:stack()`. `stack()` walks the triggering thread's stack (`lua_getstack` / `lua_getinfo`) and returns an array of `lua.Frame` objects, index 1 being the frame the hook fired in. The walk needs the thread paused at the hook, so a `_hook_active` flag on the info table (cleared when `hook_dispatch` finishes) makes a stored info table's `stack()` call raise instead of walking a stale stack.

Frames carry the thread and level, so the host can read and write the frame's active locals (`frame:getLocal` / `frame:setLocal` via `lua_getlocal` / `lua_setlocal`), its function's upvalues (`getUpvalue` / `setUpvalue`), and evaluate code in the frame's context (`frame:eval`): the eval chunk runs with a fresh environment seeded with the frame's locals and upvalues, chained via `__index`/`__newindex` to the frame function's environment (`lua_getfenv`), and assignments to existing locals/upvalues are written back with `lua_setlocal` / `lua_setupvalue` after the chunk runs — so `frame:eval("x = 42")` changes the running program. Frames are only valid while the callback runs; like the fields, they resolve the owning `lua.State` through the guest registry (shared by all threads), which also lets them work on coroutine frames.
